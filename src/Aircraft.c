...
void aircraft_GetRxInput()
{
	...
	roll_Input  = RATE_SCALE * mapVal(
			channelPulseWidth_us[CHANNEL_1_ROLL],
			channel_Min[CHANNEL_1_ROLL],
			channel_Max[CHANNEL_1_ROLL],
			-20.0f,
			20.0f
	);

	pitch_Input = RATE_SCALE * mapVal(
			channelPulseWidth_us[CHANNEL_2_PITCH],
			channel_Min[CHANNEL_2_PITCH],
			channel_Max[CHANNEL_2_PITCH],
			-20.0f,
			20.0f
	);

	yaw_Input  	= RATE_SCALE * mapVal(
			channelPulseWidth_us[CHANNEL_3_YAW],
			channel_Min[CHANNEL_3_YAW],
			channel_Max[CHANNEL_3_YAW],
			-90.0f,
			90.0f
	);

	// (%) Limit top-end so controller can work
	throt_Input = mapVal(
			channelPulseWidth_us[CHANNEL_4_THOT],
...
{
	return ((channel_Min[CHANNEL_1_ROLL] < 1400.0f && channel_Max[CHANNEL_1_ROLL] > 1800.0f) &&
			(channel_Min[CHANNEL_2_PITCH] < 1400.0f && channel_Max[CHANNEL_2_PITCH] > 1800.0f) &&
			(channel_Min[CHANNEL_3_YAW] < 1400.0f && channel_Max[CHANNEL_3_YAW] > 1800.0f) &&
			(channel_Min[CHANNEL_4_THOT] < 1400.0f && channel_Max[CHANNEL_4_THOT] > 1800.0f));
}
...
{
//	TODO: Replace magic numbers with something better.
	return ((throt_Input < MIN_THROT && yaw_Input > 8.5f*RATE_SCALE) &&
			(fabsf(roll_Input) < 5.0f*RATE_SCALE && fabsf(pitch_Input) < 5.0f*RATE_SCALE));
}
...
{
	static float newMotorVal[MOTOR_NUM] = {0.0f};
 ...
 *  Turn an LED on
 */
void aircraft_Set_LED(int LED)
{
	aircraft_WriteLED(LED, GPIO_PIN_SET);
}

/*
 *  Turn an LED off
 */
void aircraft_Reset_LED(int LED)
{
	aircraft_WriteLED(LED, GPIO_PIN_RESET);
}
 ...
 *  Control an LED
 */
void aircraft_FlashLED(int LED, uint8_t freq)
{
